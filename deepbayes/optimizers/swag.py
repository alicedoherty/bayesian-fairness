...
class StochasticWeightAveragingGaussian(optimizer.Optimizer):
    ...
    def get_posterior(self):
        self.weights_stack = np.asarray(self.weights_stack)
        ws = self.weights_stack[random.sample(range(len(self.weights_stack)), 500)]#self.weights_stack[0:250]
        ...
        for i in ws:
            ...
            if(not self.full_covar):
                ...
                var.append(tf.math.sqrt(tf.math.reduce_std(tf.stack(i), axis=0)))
            else:
